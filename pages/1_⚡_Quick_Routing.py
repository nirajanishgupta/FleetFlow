...
with st.sidebar:
    ...
    plant_lat = st.number_input("Latitude", value=28.86697041, format="%.8f")
    plant_lon = st.number_input("Longitude", value=77.25606851, format="%.8f")
    plant_name = st.text_input("Name", value="Distribution Center")
...
if uploaded and run:
    ...
    if file_size > MAX_FILE_SIZE:
        st.error(f"❌ File too large: {file_size / 1024 / 1024:.1f} MB (max 10 MB)")
        st.info("💡 Tip: For large datasets (> 10 MB), consider splitting into multiple files or filtering data")
        st.stop()
    ...
    with st.spinner("Running Clarke-Wright algorithm..."):
        ...
        df = df.dropna(subset=['Buyer_Outlet_ID'])
        df = df[df['Buyer_Outlet_ID'].astype(str).str.strip() != '']
        df = df[df['Buyer_Outlet_ID'] != 'vehicle type']  # ponytail: dirty CSV filter, remove if source cleaned

        # Check for duplicates
        duplicates = df['Buyer_Outlet_ID'].duplicated()
        if duplicates.any():
            dup_count = duplicates.sum()
            st.warning(f"⚠️ Found {dup_count} duplicate store IDs - keeping first occurrence only")
            df = df.drop_duplicates(subset=['Buyer_Outlet_ID'], keep='first')
        ...
        if invalid_lat.any() or invalid_lon.any():
            ...
            st.error(f"❌ Found {invalid_count} stores with invalid coordinates (lat: -90 to 90, lon: -180 to 180)")
            st.stop()

        # Validate demand
        invalid_demand = (df['Demand_Crates'] <= 0)
        if invalid_demand.any():
            st.error(f"❌ Found {invalid_demand.sum()} stores with invalid demand (<= 0 crates)")
            st.stop()

        if len(df) == 0:
            st.error("❌ No valid stores found in CSV after filtering")
            st.stop()

        if len(df) < original_count:
            st.info(f"ℹ️ Filtered {original_count - len(df)} invalid rows, processing {len(df)} valid stores")

        stores = []
        for _, row in df.iterrows():
            stores.append({
                'id': str(row['Buyer_Outlet_ID']),
                'name': str(row['Buyer_Outlet_Name']),
                'lat': float(row['Latitude']),
                'lon': float(row['Longitude']),
                'crates': int(row['Demand_Crates'])
            })
        ...
        output = {
            ...
            'kpis': {
                'routes': len(routes),
                'stores': sum(r['drops'] for r in routes),
...
if st.session_state.results:
    ...
    st.code("""Buyer_Outlet_ID,Buyer_Outlet_Name,Latitude,Longitude,Demand_Crates
1234567,Store Name,28.6951,77.1714,25
7654321,Another Store,28.7044,77.1150,30""")
